...
* **Action Items:**
    * [c] Add a "Join Board by ID" button on the main dashboard page that triggers an input modal box.
    * [c] Fetch board metadata on submission: 
...
* **Action Items:**
    * [c] Build an "Admin / Connected Users" sidebar panel visible exclusively to the board owner to toggle user write permissions ("Full Access" vs "View-Only").
    * [c] Implement client-side UI conditioning: if a user is `view-only`, remove the `Toolbar`, page manipulation options (adding/reordering pages), hidden utility features (`Cleanup`, `Suggestions`), and globally disable canvas interaction listeners.
