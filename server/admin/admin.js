...
(function () {
  ...
  // Everything this panel renders is user-authored: names, emails and board
  // titles come straight from open registration. They were interpolated into
  // innerHTML unescaped, so signing up as `<img src=x onerror=...>` ran script
  // in the session of any admin who opened this page.
  ...
  // Colours land inside a style attribute, where entity escaping alone would
  // still permit extra CSS declarations. Anything unexpected falls back.
  function safeColor(value) {
    return /^#[0-9a-fA-F]{3,8}$/.test(String(value || "")) ? value : "#6B7280";
  }
  ...
  async function loadDashboardStats() {
    try {
      ...
      document.getElementById("stat-sockets").textContent =
        data.activeSocketCount;
      ...
      badge.textContent = data.redisConnected
        ? "Connected & Healthy"
        ...
        tr.innerHTML = `
          <td>
            <div style="display:flex; align-items:center; gap:8px;">
              <span class="avatar-dot" style="background-color:${safeColor(u.avatar_color)}"></span>
          ...
          <td>
            <div class="table-actions">
              <button class="btn btn-secondary btn-xs update-role-btn" data-id="${escapeHtml(u._id)}" data-role="${escapeHtml(u.role || "user")}">
                ${(u.role || "user") === "admin" ? "Demote to User" : "Promote to Admin"}
              </button>
              <button class="btn btn-danger btn-xs delete-user-btn" data-id="${escapeHtml(u._id)}">Delete</button>
            </div>
          </td>
        `;
  ...
  async function loadBoardsList() {
    try {
      ...
      boards.forEach((b) => {
        const tr = document.createElement("tr");
        tr.innerHTML = `
          <td><strong>${escapeHtml(b.title || "Untitled Board")}</strong></td>
          <td>
            ${
              b.owner
                ? `
              <div>${escapeHtml(b.owner.name)}</div>
              <div style="font-size:11px; color:#64748B;">${escapeHtml(b.owner.email)}</div>
            `
          ...
          <td>
            <button class="btn btn-danger btn-xs delete-board-btn" data-id="${escapeHtml(b._id)}">Delete Board</button>
          </td>
        `;
        tbody.appendChild(tr);
      });
      ...
      document.querySelectorAll(".delete-board-btn").forEach((btn) => {
        btn.addEventListener("click", () =>
          deleteBoard(btn.getAttribute("data-id")),
        );
      });
    } catch (err) {
      showError(err.message);
  ...
  async function updateRole(userId, currentRole) {
    ...
    if (
      !confirm(
        `Are you sure you want to promote/demote this user to ${newRole.toUpperCase()}?`,
      )
    )
      return;
  ...
  async function deleteUser(userId) {
    if (
      !confirm(
        "WARNING: Deleting a user profile will permanently cascade delete all whiteboards they own. Are you sure you want to proceed?",
      )
    )
      return;
  ...
  async function deleteBoard(boardId) {
    if (
      !confirm(
        "Are you sure you want to permanently delete this whiteboard? This action is irreversible.",
      )
    )
      return;
  ...
  toggleInput.addEventListener("change", async () => {
    ...
    try {
      ...
      const data = await handleResponse(res);
      if (data) {
        document.getElementById("active-cache-mode-label").textContent =
          data.activeCacheMode.toUpperCase();
        document.getElementById("toggle-status-text").textContent =
          data.activeCacheMode === "redis"
            ? "Using Redis Cache"
            : "Using Memory Fallback";
      }
