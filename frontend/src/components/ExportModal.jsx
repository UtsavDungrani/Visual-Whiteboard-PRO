...
export default function ExportModal({
  ...
  const handleOptimizedExport = async () => {
    ...
    if (!window.showDirectoryPicker) {
      alert(
        "Your browser does not support the File System Access API. Please use a modern desktop browser like Chrome or Edge over HTTPS.",
      );
      return;
    }
    ...
    try {
      ...
      const subDirHandle = await rootDirHandle.getDirectoryHandle(folderName, {
        create: true,
      });
      ...
      const shouldCreatePdf = window.confirm(
        `Successfully saved ${selectedPages.length} images to "${folderName}".\n\nWould you like to generate a compiled PDF document from these images as well?`,
      );
        ...
        <div className="modal-body-split">
          ...
          <div className="modal-left-pane">
            <div className="pane-section-header">
              <span className="modal-section-title">Select Pages</span>
              <button
                  ...
                  >
                    ...
                    <div className="page-card-details">
                      ...
                      <span className="page-card-sub">
                        {p.canvas_state?.objects?.length || 0} elements
                      </span>
                    </div>
                  </div>
                );
              ...
              {activeTab === "pdf" ? (
                ...
                <div className="options-form">
                  ...
                  <div className="form-group">
                    ...
                    <select
                      value={pdfResolution}
                      onChange={(e) =>
                        setPdfResolution(parseInt(e.target.value, 10))
                      }
                      className="form-select"
                    >
                      <option value={1}>
                        Standard Web Resolution (72 DPI)
                      </option>
                      <option value={2}>
                        Print / High Definition (150 DPI)
                      </option>
                      <option value={4}>
                        Ultra Presentation Vector Quality (300 DPI)
                      </option>
                    </select>
                  </div>

                  <div
                    className="export-actions-row"
                ...
                <div className="options-form">
                  <div className="form-group">
                    ...
                    >
                      <option value="single">
                        Single Web Page (Sequential Sections)
                      </option>
                      <option value="zip">
                        Separate HTML Sheets (ZIP Bundle)
                      </option>
                    </select>
                  </div>
